...
class FrontendAjax /*extends \CMBShortcodes\Main */ {
	...
	public function __construct() {
		$shortcode = $_REQUEST["atts"]["name"];
		$shortcode_config_file = wp_normalize_path(dirname( __FILE__, 2).'/shortcodes/'.$shortcode.'/scu-config.ini');
		$config_array = parse_ini_file($shortcode_config_file, true);
		if($config_array["type"]["ajax"]) {
			$sc_ajax_handler_file = wp_normalize_path(dirname( __FILE__, 2).'/shortcodes/'.$shortcode.'/scu-ajax-handler.php');
