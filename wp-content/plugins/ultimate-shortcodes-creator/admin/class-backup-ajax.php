...
class BackupAjax {	
	...
	private function _install_shortcode($shortcode_file_path) {
		...
		$file_name_unique = wp_unique_filename(\SCU\PATH.'shortcodes/', $file_name);
		$shortcode_dir = wp_normalize_path(\SCU\PATH.'shortcodes/'.$file_name_unique);
	...
	public function backup_download() {
		...
		$sc_path = realpath(dirname( __FILE__, 2).'/shortcodes').DIRECTORY_SEPARATOR.'*';
