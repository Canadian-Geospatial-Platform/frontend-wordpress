...
class Shortcode_List_Table extends \WP_List_Table {
	...
	private function setShortcodesData() {
		...
		foreach ($availableShortcodes as $key => $shortcode) {			
			$ini_file = wp_normalize_path(\SCU\PATH.'/shortcodes/'.basename($shortcode).'/scu-config.ini');
		...
		switch ($this->current_action()) {			
			case 'delete':				
				...
				foreach ($shortcodesToDoAction as $key => $shortcode) {
					...
					$src_dir = $wp_filesystem->find_folder(\SCU\PATH.'shortcodes/'.$shortcode);
			...
			case 'enable':
				foreach ($shortcodesToDoAction as $key => $shortcode) {								
					$iniFileName = \SCU\PATH.'shortcodes/'.basename($shortcode).'/scu-config.ini';
			...
			case 'export':
				...
				if(count($shortcodesToDoAction) == 1) {					
					$source_dir = realpath(\SCU\PATH.'shortcodes/'.basename($shortcodesToDoAction[0]));			
					...
					foreach ($shortcodesToDoAction as $key => $shortcode) {		
						$source = realpath(\SCU\PATH.'shortcodes/'.basename($shortcode));			
