...
define ('SCU\PATH', plugin_dir_path( __FILE__ ));					// PATH del plugin
define ('SCU\VER', '1.9');											// Version
...
//wp_die(print_r(PLUGIN_NAME));
// Needed for deactivate_plugins function
require_once( ABSPATH . 'wp-admin/includes/plugin.php' );
// All stuff for installer and uninstaller
require_once( wp_normalize_path(dirname( __FILE__ ).'/includes/class-installer.php'));
...
class Main {	
	...
	//$url = plugins_url( '', __FILE__ );					// URL del plugin
	//const PATH = plugin_dir_path( __FILE__ ); 					// PATH del plugin
	//const PLUGIN_NAME = dirname(plugin_basename(__FILE__));		// Plugin Name
	//const PLUGIN_PHPFILE = plugin_basename(__FILE__);			// Main File Name with relative path (needed for deactivate_plugins)
