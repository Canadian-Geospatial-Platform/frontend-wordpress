    ...
    class Search_Results_View
    {
        ...
        public function get_header_html($collectionProps)
        {
            if ($collectionProps['total_posts'] >= 1) {
                ...
                $html = '<h2>' . $collectionProps['total_posts'] . ' ' . __('results found for', 'pauple-helpie') . ' ';
                $html .= '&lsquo;' . __($collectionProps['search_query'], 'pauple-helpie') . '&rsquo; </h2>';
            } else {
                $html = '<h2> ' . __($collectionProps['empty_search_result_label'], 'pauple-helpie') . ' </h2>';
            }
        ...
        protected function get_anchor_html($itemProps)
        {
            ...
            $html .= '<a ' . $href . ' class="item-title">' . __($itemProps['title'], 'pauple-helpie') . '</a>';
            $html .= ' <span class="item-in">' . __('in', 'pauple-helpie') . '</span>';
            $html .= ' <span class="item-cat_name">' . __($itemProps['category'], 'pauple-helpie') . '</span>';
            $html .= '</span>';
        ...
        protected function get_meta_html($itemProps)
        {
            ...
            $html .= '<i class="calendar alternate outline icon"></i> <span class="meta-value">' . __($itemProps['date'], 'pauple-helpie') . '</span>';
            $html .= '</span>  ';
        ...
        protected function get_description_html($itemProps, $collectionProps)
        {
            ...
            $html .= '<p>' . __($content, 'pauple-helpie') . '</p>';
            $html .= '</div>';
        ...
        protected function get_article_tags($tags)
        {
            ...
            foreach ($tags as $tag) {
                if (stripos($tag, 'strong') !== false) {
                    $html .= "<div class='ui blue label'>" . __($tag, 'pauple-helpie') . "</div>";
                } else {
                    $html .= "<div class='ui basic label'>" . __($tag, 'pauple-helpie') . "</div>";
                }
            }
            $html .= "</div>";
